...
/**
 * Lecture des justificatifs. Leur dépôt, lui, se fait avec la dépense qui les porte
 * (`POST /api/expenses/file`) : un fichier déposé seul n'appartiendrait à rien, ne serait plus
 * lisible par personne, et aucune purge ne saurait le rattraper.
 */
export const uploadRoutes: FastifyPluginAsync<UploadRoutesOptions> = async (app, { storage, expenseService }) => {
  /**
